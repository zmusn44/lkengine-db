 ...
 */
public class SaveDocMSSQL implements SaveDoc {
     ...
     * @param doc 要存盘的文档
     * @param conn 指定数据库链接对像，如果为当前链接则可以传null空值
     * @param extendTableName 用来指定扩展数据存储表的名称，默认可以给定为xmldata即存在xml字段中
     * @return 返回非负数表示存盘成功
    ...
    public int save(Connection conn, Document doc, String tableName, String extendTableName) throws Exception {
        ...
        for (String fdName : fdList.keySet()) {
            ...
            if (separator.equals("") && fdValue.equals("")) {
                fdValue = "NULL";
            }
            //解决数据库不支持部分特殊字符的问题
    ...
    private String getFieldSeparator(String typeName) {
        ...
        String typeList = fieldTypeList.toLowerCase(); // 从配置值中获取
